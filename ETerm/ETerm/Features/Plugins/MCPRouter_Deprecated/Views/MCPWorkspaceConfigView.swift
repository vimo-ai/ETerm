//
//  MCPWorkspaceConfigView.swift
//  ETerm
//
//  MCP Router Workspace 管理视图 - 使用 IPC 调用
//
...
struct MCPWorkspaceConfigView: View {
    @StateObject private var viewModel = MCPWorkspaceViewModel()
    ...
    var body: some View {
        ...
        .sheet(item: $selectedWorkspace) { workspace in
            MCPWorkspaceDetailView(
                ...
                onUpdate: { Task { await viewModel.refresh() } }
            )
        }
        .task {
...
@MainActor
final class MCPWorkspaceViewModel: ObservableObject {
    @Published private(set) var workspaces: [MCPWorkspaceDTO] = []
    ...
    func refresh() async {
        do {
            ...
            print("[MCPWorkspace] Failed to refresh: \(error)")
        }
    }
}
        ...
        VStack(spacing: 0) {
            ...
            HStack {
                ...
                if !workspace.isDefault && (!workspace.serverOverrides.isEmpty || !workspace.flattenOverrides.isEmpty) {
                    Button("重置") {
                        Task {
                            try? await client.resetWorkspaceOverrides(token: workspace.token)
                            onUpdate()
                            dismiss()
                        }
                    }
                    ...
                    LazyVStack(spacing: 8) {
                        ForEach(servers) { server in
                            ServerToggleCardView(
                                ...
                                onToggle: { enabled in
                                    Task {
                                        try? await client.setWorkspaceServerEnabled(
                                ...
                                onFlattenToggle: { flatten in
                                    Task {
                                        try? await client.setWorkspaceFlattenMode(
                                            token: workspace.token,
                                            serverName: server.name,
                                            flatten: flatten
                                        )
