//
//  MCPJSONImportView.swift
//  ETerm
//
//  MCP Router JSON 导入界面 - 使用 IPC 调用
//
...
struct MCPJSONImportView: View {
    ...
    @State private var existingServers: [MCPServerConfigDTO] = []
    ...
    private func loadExistingServers() async {
        do {
            existingServers = try await client.listServers()
        } catch {
            existingServers = []
        }
    }
    ...
    private func parseServerConfig(name: String, config: [String: Any]) -> MCPServerConfigDTO? {
        let type: String
        ...
        if type == "http" {
            ...
            return MCPServerConfigDTO.http(name: name, url: url, headers: headers)
        } else {
            ...
            return MCPServerConfigDTO.stdio(name: name, command: command, args: args, env: env)
        }
    }
}
