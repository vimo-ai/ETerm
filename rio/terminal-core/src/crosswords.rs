...
use crate::square::{Hyperlink, LineLength, Square};
...
pub type NamedColor = crate::colors::NamedColor;
    ...
    U: EventListener,
{
    ...
    pub(crate) semantic_escape_chars: String,
    pub grid: Grid<Square>,
    pub(crate) inactive_grid: Grid<Square>,
...
impl<U: EventListener> Crosswords<U> {
    ...
    pub fn expand_wide(&self, mut pos: Pos, direction: Direction) -> Pos {
        ...
        match direction {
            Direction::Right
                if flags.contains(crate::square::Flags::LEADING_WIDE_CHAR_SPACER) =>
            {
                pos.col = Column(1);
                pos.row += 1;
            }
            Direction::Right if flags.contains(crate::square::Flags::WIDE_CHAR) => {
                pos.col = std::cmp::min(pos.col + 1, self.grid.last_column());
            }
            Direction::Left
                if flags.intersects(
                    crate::square::Flags::WIDE_CHAR | crate::square::Flags::WIDE_CHAR_SPACER,
                ) =>
            {
                if flags.contains(crate::square::Flags::WIDE_CHAR_SPACER) {
                    pos.col -= 1;
                }
                ...
                if self.grid[prev]
                    .flags
                    .contains(crate::square::Flags::LEADING_WIDE_CHAR_SPACER)
                {
                    pos = prev;
                }
            }
    ...
    pub fn wrapline(&mut self) {
        ...
        self.grid
            .cursor_cell()
            .flags
            .insert(crate::square::Flags::WRAPLINE);
    ...
    pub fn write_at_cursor(&mut self, c: char) {
        ...
        if cursor_square
            .flags
            .intersects(crate::square::Flags::WIDE_CHAR | crate::square::Flags::WIDE_CHAR_SPACER)
        {
            // Remove wide char and spacer.
            let wide = cursor_square.flags.contains(crate::square::Flags::WIDE_CHAR);
            let point = self.grid.cursor.pos;
            if wide && point.col < self.grid.last_column() {
                self.grid[point.row][point.col + 1]
                    .flags
                    .remove(crate::square::Flags::WIDE_CHAR_SPACER);
            } else if point.col > 0 {
            ...
            if point.col <= 1 && point.row != self.grid.topmost_line() {
                ...
                self.grid[point.row - 1i32][column]
                    .flags
                    .remove(crate::square::Flags::LEADING_WIDE_CHAR_SPACER);
            }
    ...
    pub fn cursor(&self) -> CursorState {
        ...
        if self.grid[pos]
            .flags
            .contains(crate::square::Flags::WIDE_CHAR_SPACER)
        {
            pos.col -= 1;
        }
    ...
    fn line_to_string(
        ...
        for column in (cols.start.0..line_length.0).map(Column::from) {
            ...
            if !cell.flags.intersects(
                crate::square::Flags::WIDE_CHAR_SPACER | crate::square::Flags::LEADING_WIDE_CHAR_SPACER,
            ) {
        ...
        if cols.end >= self.grid.columns() - 1
            && (line_length.0 == 0
                || !self.grid[line][line_length - 1]
                    .flags
                    .contains(crate::square::Flags::WRAPLINE))
        {
            text.push('\n');
        }
...
impl<U: EventListener> Handler for Crosswords<U> {
    ...
    fn terminal_attribute(&mut self, attr: Attr) {
        ...
        match attr {
            ...
            Attr::Reset => {
                ...
                cursor.template.flags = crate::square::Flags::empty();
                cursor.template.set_underline_color(None);
            }
            Attr::Reverse => cursor.template.flags.insert(crate::square::Flags::INVERSE),
            Attr::CancelReverse => cursor.template.flags.remove(crate::square::Flags::INVERSE),
            Attr::Bold => cursor.template.flags.insert(crate::square::Flags::BOLD),
            Attr::CancelBold => cursor.template.flags.remove(crate::square::Flags::BOLD),
            Attr::Dim => cursor.template.flags.insert(crate::square::Flags::DIM),
            Attr::CancelBoldDim => cursor
                .template
                .flags
                .remove(crate::square::Flags::BOLD | crate::square::Flags::DIM),
            Attr::Italic => cursor.template.flags.insert(crate::square::Flags::ITALIC),
            Attr::CancelItalic => cursor.template.flags.remove(crate::square::Flags::ITALIC),
            Attr::Underline => {
                cursor.template.flags.remove(crate::square::Flags::ALL_UNDERLINES);
                cursor.template.flags.insert(crate::square::Flags::UNDERLINE);
            }
            Attr::DoubleUnderline => {
                ...
                cursor
                    .template
                    .flags
                    .insert(crate::square::Flags::DOUBLE_UNDERLINE);
            }
            Attr::Undercurl => {
                cursor.template.flags.remove(crate::square::Flags::ALL_UNDERLINES);
                cursor.template.flags.insert(crate::square::Flags::UNDERCURL);
            }
            Attr::DottedUnderline => {
                ...
                cursor
                    .template
                    .flags
                    .insert(crate::square::Flags::DOTTED_UNDERLINE);
            }
            Attr::DashedUnderline => {
                ...
                cursor
                    .template
                    .flags
                    .insert(crate::square::Flags::DASHED_UNDERLINE);
            }
            ...
            Attr::Hidden => cursor.template.flags.insert(crate::square::Flags::HIDDEN),
            Attr::CancelHidden => cursor.template.flags.remove(crate::square::Flags::HIDDEN),
            Attr::Strike => cursor.template.flags.insert(crate::square::Flags::STRIKEOUT),
            Attr::CancelStrike => cursor.template.flags.remove(crate::square::Flags::STRIKEOUT),
            // _ => {
    ...
    fn shell_command_execute(&mut self, command: Option<&str>, git_branch: Option<&str>) {
        if let Some(cmd) = command {
            trace!("Shell command execute: {}", cmd);
            self.event_proxy
                .send_event(RioEvent::CommandExecuted(cmd.to_string()), self.window_id);
            self.event_proxy.send_event(
                RioEvent::ShellCommandStarted {
                    command: cmd.to_string(),
                    cwd: self.current_directory.as_ref().map(|p| p.to_string_lossy().to_string()),
    ...
    fn shell_command_finished(&mut self, exit_code: Option<u8>) {
        self.event_proxy.send_event(
    ...
    fn configure_charset(
        &mut self,
        index: crate::pos::CharsetIndex,
        charset: crate::pos::StandardCharset,
    ) {
    ...
    fn input(&mut self, c: char) {
        ...
        if width == 0 {
            ...
            if c == '\u{FE0F}' {
                ...
                if !cell.flags.contains(crate::square::Flags::WIDE_CHAR) {
                    let columns = self.grid.columns();
                    // 确保有空间放置 WIDE_CHAR_SPACER
                    if column.0 + 1 < columns {
                        // 将当前 cell 标记为 WIDE_CHAR
                        self.grid[row][column].flags.insert(crate::square::Flags::WIDE_CHAR);
                        ...
                        self.grid[row][next_col].flags = crate::square::Flags::WIDE_CHAR_SPACER;
        ...
        if width == 1 {
            ...
            if self.grid.cursor.pos.col + 1 >= columns {
                if self.mode.contains(Mode::LINE_WRAP) {
                    ...
                    self.grid
                        ...
                        .template
                        .flags
                        .insert(crate::square::Flags::LEADING_WIDE_CHAR_SPACER);
            ...
            self.grid
                ...
                .template
                .flags
                .insert(crate::square::Flags::WIDE_CHAR);
            self.write_at_cursor(c);
            self.grid
                .cursor
                .template
                .flags
                .remove(crate::square::Flags::WIDE_CHAR);
            ...
            self.grid
                ...
                .template
                .flags
                .insert(crate::square::Flags::WIDE_CHAR_SPACER);
            self.write_at_cursor(' ');
            self.grid
                .cursor
                .template
    ...
    fn insert_graphic(&mut self, graphic: GraphicData, palette: Option<Vec<ColorRgb>>) {
        ...
        let graphic = if graphic.resize.is_some() { return; } else { graphic };
...
impl<U: EventListener> Crosswords<U> {
    ...
    pub fn start_search(
        ...
        // Create RegexSearch
        let mut regex = crate::search::RegexSearch::new(&pattern)
    ...
    fn find_all_matches(
        &self,
        regex: &mut crate::search::RegexSearch,
        max_lines: Option<usize>,
        ...
        let iter = crate::search::RegexIter::new(start, end, Direction::Right, self, regex);
        for m in iter {
            matches.push(m);
