...
fn fs_main(input: VertexOutput) -> @location(0) vec4<f32> {
    ...
    if input.color_layer > 0 {
        out = textureSampleLevel(color_texture, color_sampler, input.f_uv, 0.0);
    }

    if input.mask_layer > 0 {
        out = vec4<f32>(out.xyz, input.f_color.a * textureSampleLevel(mask_texture, mask_sampler, input.f_uv, 0.0).x);
    }
