...
//  AICliKit
//
//  Gemini CLI Provider - 通过 Unix Socket 接收 Gemini CLI Hook 事件
//
//  Gemini CLI hooks 通过 settings.json 配置：
//  {
//    "hooks": {
//      "SessionStart": "echo '{\"event_type\":\"SessionStart\",...}' | nc -U ~/.vimo/eterm/sockets/gemini.sock",
//      ...
//    }
//  }
...
/// Gemini Provider - 实现 AICliProvider 协议
///
/// 通过 Unix Domain Socket 接收 Gemini CLI hooks 事件，
/// 将原始事件转换为标准 AICliEvent。
@MainActor
public final class GeminiProvider: AICliProvider {
    public static let providerId = "gemini"

    public static let capabilities = AICliCapabilities.gemini

    public var onEvent: ((AICliEvent) -> Void)?

    private var socketServer: GeminiSocketServer?
    ...
    public func start(config: AICliProviderConfig) {
        ...
        socketServer = GeminiSocketServer()
        ...
        let socketPath = config.socketDirectory + "/gemini.sock"
    ...
    /// 将 Gemini 原始事件映射为标准 AICliEvent
    private func mapEvent(_ raw: GeminiHookEvent) -> AICliEvent? {
        ...
        case "BeforeTool":
            // 如果是 decision 类型，映射为 permissionRequest
            if raw.decision == true {
...
struct GeminiAnyCodable: Codable {
    ...
    func encode(to encoder: Encoder) throws {
        ...
        case let array as [Any]:
            try container.encode(array.map { GeminiAnyCodable($0) })
        case let dict as [String: Any]:
            try container.encode(dict.mapValues { GeminiAnyCodable($0) })
        default:
...
// MARK: - Gemini Socket Server (内部实现)

/// Socket Server - 接收来自 Gemini Hook 的调用
final class GeminiSocketServer {
    ...
    /// 事件回调
    var onEvent: ((GeminiHookEvent) -> Void)?
    ...
    private func startAcceptingConnections() {
        acceptQueue = DispatchQueue(label: "com.eterm.aicli.gemini-socket")
